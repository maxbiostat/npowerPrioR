#' Build grid of values for the tempering scalar along with the
#'  (first and second) derivatives of the log-normalising constant.
#'
...
#' @export build_grid_derivOnly
#' @seealso \code{\link[npowerPrioR]{build_grid}}
#'
build_grid_derivOnly <- function(compiled.model.prior, eps = .01,  M = 10, J = 15, v1 = 5, v2 = 10, stan.list, pars,
                                 niter = 5000, strict = FALSE,  step_size = 0.99, tdepth = 15){
  
  f0 <- get_deriv_only(a = 0.0, compiled.model.prior = compiled.model.prior,  stan.list = stan.list, pars = pars,
                       strict = strict, niter = niter, tdepth = tdepth, step_size = step_size) ## need to run for a_0 = 0 to get parameter summaries.
  
  ## Preliminary checking 
  fm <- get_deriv_only(a = eps, compiled.model.prior = compiled.model.prior, stan.list = stan.list, pars = pars,
                       strict = strict, niter = niter, tdepth = tdepth, step_size = step_size)
  fM <- get_deriv_only(a = M, compiled.model.prior = compiled.model.prior, stan.list = stan.list, pars = pars,
                       strict = strict, niter = niter, tdepth = tdepth, step_size = step_size)
  ...
  if(same_sign){ ## if they are the same sign, can construct a regularly spaced grid
    ...
    all.outs[3:J] <- lapply(a0_grid, get_deriv_only, compiled.model.prior = compiled.model.prior, stan.list = stan.list, pars = pars,
                            strict = strict, niter = niter, tdepth = tdepth, step_size = step_size)
    ...
    while(counter > 0 && delta > v1*eps){
      lz <- get_deriv_only(a = z, compiled.model.prior = compiled.model.prior, stan.list = stan.list, pars = pars,
                           strict = strict, niter = niter, tdepth = tdepth, step_size = step_size)
      a0s <- c(a0s, z)
